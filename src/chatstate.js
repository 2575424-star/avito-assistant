...
function cpaPromptLine(state) {
  ...
  // Важно не отбить у модели желание просить номер: просьба сама чат платным не делает,
  // платным его делает номер от клиента — а это и есть желаемый результат.
  return `СОСТОЯНИЕ ЧАТА: реплик ${state.turns}, чат ещё не платный. Твой ответ станет репликой №${state.turns + 1}${left <= 1 ? ' — на 5-й реплике чат станет платным' : ''}.`
    + ' Сам не отправляй адрес салона, часы работы, ссылки и свои контакты, если клиент о них не спросил.'
    + ' Попросить номер телефона клиента под конкретный шаг (звонок с расчётом, осмотр, подбор) можно и нужно: сама просьба чат платным не делает, а номер от клиента — желаемый результат.'
    + (state.clientTurns >= 2
      ? ' Клиент пишет уже не первый раз — предложи конкретный шаг и попроси номер, если ещё не просил и он не отказывался от звонков.'
      : ' Это первый ответ: если уместно, одним сообщением ответь на вопрос и сразу предложи оставить номер для подробной консультации.');
}
