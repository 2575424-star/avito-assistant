...
- **Сравнение моделей** — несколько моделей (OpenAI, а через OpenRouter — Claude, Gemini, DeepSeek и др.) отвечают параллельно на одно и то же сообщение; ответы рядом, таблица: доля 👍, токены, время ответа.
