...
async function renderGptLab() {
  ...
  const selectedModels = new Set(['gpt-6-luna']);
  ...
  view().innerHTML = `<div class="page-head"><h1>Лаборатория GPT</h1></div>
    <p class="muted">Егор, менеджер отдела продаж автосалона InDrive. Короткая инструкция egor-2. Luna и, по желанию, GPT-4o mini. Ответы только здесь — клиентам ничего не отправляется.</p>
    ...
    <div class="row">${cfg.models.map(m=>`<label class="badge"><input type="checkbox" data-gmodel="${esc(m.model)}" ${m.model==='gpt-6-luna'?'checked':''} ${m.ready?'':'disabled'}> ${esc(m.label)} · ${m.ready ? 'ключ подключён: '+esc(m.profile) : 'ключ не подключён'}</label>`).join('')}</div>
    ...
    <div class="row" style="margin-top:16px"><button class="btn primary" id="gStart">Сгенерировать ответы</button><button class="btn" id="gStop" hidden>Остановить</button><span id="gProgress" role="status"></span></div><p class="small muted" id="gCount">Выберите вопросы</p></div>
  ...
  const update = () => { $('#gCount').textContent = `Выбрано вопросов: ${selected.size} · вариантов ответа: ${selected.size*selectedModels.size*2}. Многоходовые вопросы включают несколько сообщений.`; $('#gStart').disabled = busy || !selected.size || !selectedModels.size || cfg.models.some(m=>selectedModels.has(m.model)&&!m.ready); };
  $$('[data-gmodel]').forEach(x=>x.addEventListener('change',()=>{x.checked?selectedModels.add(x.dataset.gmodel):selectedModels.delete(x.dataset.gmodel);update();}));
  ...
  async function refresh() {
    ...
    try {
      ...
      <div class="gpt-answer-grid">${cfg.strategies.flatMap(s=>(d.models.length?d.models:cfg.models.filter(m=>selectedModels.has(m.model))).map(m=>{
  ...
  $('#gStart').onclick=async()=>{busy=true;update();try{const j=await api('/api/gpt-lab/run',{body:{caseIds:[...selected],models:[...selectedModels]}});batch=j.batch;await poll();}catch(e){busy=false;update();toast(e.message,true);}};
