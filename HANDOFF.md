...
## 3а. Обучение агента на реальных чатах (сентябрь 2026)

Добавлено по запросу Павла: сначала учим агента на истории, ничего не отправляя.

- **Тестовый режим** (`send_enabled=0`, по умолчанию): `avito.sendMessage` и `avito.markRead` бросают ошибку, пока не включён боевой режим (Настройки → Агент). Если в тестовом режиме включить AI, на новые сообщения агент пишет черновики (`agent_runs.kind='shadow'`), они видны в чате пунктиром. Быстрый ответ не уходит, шаблон сохраняется черновиком.
- **Загрузка всей истории** (`src/history.js`, `POST /api/archive/import`): все чаты постранично, все сообщения каждого чата (по 100, до 3000). Если список чатов обрывается (у Авито может быть ограничение глубины `offset`), чаты добираются запросами `chats?item_ids=` по объявлениям из базы. Повторный запуск пропускает неизменившиеся чаты (`chats.history_loaded`). Телефоны из истории становятся лидами датой сообщения, без уведомлений.
- **Статистика продавцов** (`GET /api/archive/stats`): входящие чаты, без ответа, медиана первого ответа (днём/ночью), доля «просил телефон», ушли после ответа, по месяцам. Ответы «ИИ Диалогов» Авито не отличает от ответов продавцов.
- **Выгрузка JSONL** (`/api/archive/export.jsonl`): одна строка — один диалог, формат как у `avito-raw-export`.
- **Разбор нейросетью** (`POST /api/archive/review`, таблицы `chat_reviews`, `reports`): оценка продавца, итог, ошибки, «как надо было ответить», FAQ; сводный отчёт с правилами. Из интерфейса правила добавляются в промпт, FAQ и примеры — в базу знаний.
- **База знаний** (`src/knowledge.js`, таблица `kb`): категории faq / company / objections / rules / example. В промпт идёт вся база, пока укладывается в `kb_budget_chars` (16 000), дальше — самые близкие к вопросу клиента записи (пересечение основ слов, без эмбеддингов).
- **Автомобили** (таблица `items`): `GET /core/v1/items` (название, цена, статус, ссылка) + XML-фид автозагрузки (URL из `GET /autoload/v2/profile` или вручную; сопоставление Id фида с ID Авито через `GET /autoload/v2/items/avito_ids`). Название и цену фид не перетирает. В промпт — полная карточка машины из чата и список остальных машин в продаже.
- **Прогон по истории** (`POST /api/chats/:id/replay`, `POST /api/replay-batch`): агент отвечает на каждую серию сообщений клиента, видя переписку только до этого момента; результат в `agent_runs.kind='replay'`, рядом фактический ответ продавца. Оценка 👍/👎 и исправление (`POST /api/runs/:id`), исправление сохраняется в базу как пример.
- **Сравнение моделей:** настройка `compare_models` (через запятую). Модель без приставки — OpenAI, с приставкой `openrouter:` — через OpenRouter (`openrouter_api_key`). При прогоне все модели отвечают на одно сообщение параллельно (`Promise.allSettled`), ошибка одной модели сохраняется как черновик с комментарием «Ошибка: …». В `agent_runs` пишутся `model` и `ms`; `GET /api/runs` отдаёт группы (сообщение клиента → последний ответ каждой модели) и статистику по моделям. Если модель не поддерживает JSON-режим, запрос повторяется без него.
- **Надёжность API:** при 429 до 3 повторов с ожиданием по `Retry-After`, при 5xx повторяются только GET.
- **Тесты:** `npm test` — мок Авито/OpenAI (`test/mock-server.js`) и сквозной сценарий (`test/smoke.js`).

Не сделано: расшифровка голосовых (`getVoiceFiles` + Whisper), записи звонков (`calltracking`), разбивка по сотрудникам (`hierarchy`) — см. `docs/competitors.md`.
...
4. **Надёжность:**
   - подписи вебхука у Авито нет (см. `docs/competitors.md`): вместо неё отбрасывать повторные доставки по `id` события и сравнивать секрет за постоянное время;
   - ретраи при 429/5xx сделаны; ограничения частоты на своей стороне нет;
